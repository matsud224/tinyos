...
void ip_rx(struct pktbuf_head *frm){
	//正しいヘッダかチェック
	if(frm->total < sizeof(struct ether_hdr)+sizeof(struct ip_hdr))
		goto exit;
	if(frm->total < sizeof(struct ether_hdr)+ntoh16(iphdr->ip_len) ){
		//Ethernetパディングも考慮
		goto exit;
	}
	if(iphdr->ip_v != 4)
	...
	*/
	...
	if(!((ntoh16(iphdr->ip_off) & IP_OFFMASK) == 0 && (ntoh16(iphdr->ip_off) & IP_MF) == 0)){
      ...
      for(struct fragment *fptr=info->fragmentlist;fptr!=NULL;fptr=fptr->next){
        memcpy(origin+fptr->first,((u8*)(fptr->flm->buf))+info->headerlen,fptr->last-fptr->first+1);
	...
	switch(iphdr->ip_p){
	case IPTYPE_ICMP:
		//icmp_tx(flm, iphdr, (icmp*)(((u8*)iphdr)+(iphdr->ip_hl*4)));
		break;
	case IPTYPE_TCP:
		//tcp_tx(flm, iphdr, (tcp_hdr*)(((u8*)iphdr)+(iphdr->ip_hl*4)));
		break;
	case IPTYPE_UDP:
		udp_tx(flm, iphdr, (udp_hdr*)(((u8*)iphdr)+(iphdr->ip_hl*4)));
		break;
	}
	return;
...
void ip_send(hdrstack *data, u8 *dstaddr, u8 proto){
	u32 datalen = hdrstack_totallen(data); //IPペイロード長
	u32 remainlen = datalen;
  u16 currentid = ip_getid(); //今回のパケットに付与する識別子

  //ルーティング
  u8 dstaddr_r[IP_ADDR_LEN];
